...
void mu::io_test::builder::run_1 ()
{
	...
	assert (routine->context == mu::io::debugging::context (builder.stream, 1, 1, 0, 1, 4, 3));
}
...
void mu::io_test::builder::run_2 ()
{
	...
	assert (routine1->context == mu::io::debugging::context (builder.stream, 1, 1, 0, 1, 4, 3));
	assert (routine2->context == mu::io::debugging::context (builder.stream, 1, 5, 4, 1, 8, 7));
}
...
void mu::io_test::builder::run_3 ()
{
	...
	assert (cluster->expressions.size () == 1);
	auto routine (cluster->expressions [0]);
	assert (routine->context == mu::io::debugging::context (builder.stream, 1, 1, 0, 1, 25, 24));
