...
TEST (llvmc_analyzer, two_result_branches)
{
    mu::llvmc::analyzer analyzer;
    mu::llvmc::ast::module module;
    mu::llvmc::ast::function function;
    ...
    ASSERT_EQ (parameter2, result3->value);
    ASSERT_NE (&function1->entry, result3->value->branch);
    ASSERT_EQ (1, function1->results [1].size ());
    auto result4 (function1->results [1][0]);
    ASSERT_EQ (parameter2, result4->value);
    ASSERT_NE (&function1->entry, result4->value->branch);
    ASSERT_NE (result3->value->branch, result4->value->branch);
}
