...
void mu::llvmc::function_processor::value (mu::llvmc::ast::value * value_node)
{
	...
	if (value != nullptr)
	{
		...
		value->branch = &mu::llvmc::skeleton::branch::global;
	}
	...
	try
	{
		...
		if (bits <= 1024)
		{
			...
			if (number != nullptr)
			{
				...
				if (number_l != nullptr)
				{
					constant_a->generated.push_back (new (GC) mu::llvmc::skeleton::constant_integer (constant_a->region, &mu::llvmc::skeleton::branch::global, bits, number_l->value));
...
void mu::llvmc::module_processor::constant_array (mu::llvmc::ast::constant_array * array_a)
{
	...
	if (type != nullptr)
	{
		...
		array_a->generated.push_back (new (GC) mu::llvmc::skeleton::constant_array (array_a->region, &mu::llvmc::skeleton::branch::global, array_type, initializer));
...
void mu::llvmc::module_processor::global_variable (mu::llvmc::ast::global_variable * global_variable)
{
	...
	if (values.size () == 1)
	{
		...
		if (constant != nullptr)
		{
			...
			auto skeleton (new (GC) mu::llvmc::skeleton::global_variable (global_variable->region, &mu::llvmc::skeleton::branch::global, constant));
...
void mu::llvmc::module_processor::constant_pointer_null (mu::llvmc::ast::constant_pointer_null * constant_pointer_null)
{
	...
	if (type != nullptr)
	{
		...
		if (pointer != nullptr)
		{
			...
			auto skeleton (new (GC) mu::llvmc::skeleton::constant_pointer_null (constant_pointer_null->region, &mu::llvmc::skeleton::branch::global, type));
	...
	if (existing == current_expression_generation.end ())
	{
		...
		if (!expression_a->arguments.empty ())
		{
			...
			if (global_m.error == nullptr)
			{
				...
				{
					global_m.error = new (GC) mu::core::error_string (U"Expecting target", mu::core::error_type::expecting_a_target, expression_a->arguments [0]->region);
				}
				if (global_m.error == nullptr)
				{
		...
		{
			global_m.error = new (GC) mu::core::error_string (U"Expecting a call target", mu::core::error_type::expecting_a_call_target, expression_a->region);
		}
	}
	else
	{
		global_m.error = new (GC) mu::core::error_string (U"Cycle in expressions", mu::core::error_type::cycle_in_expressions, expression_a->region);
	}
...
void mu::llvmc::function_processor::undefined (mu::llvmc::ast::undefined * node_a)
{
	...
	if (type != nullptr)
	{
		auto undefined_l (new (GC) mu::llvmc::skeleton::undefined (node_a->region, &mu::llvmc::skeleton::branch::global, type));
		node_a->assigned = true;
...
void mu::llvmc::module_processor::process_identity (mu::llvmc::ast::expression * expression_a)
{
    ...
    switch (predicate_offset)
    {
        ...
        case 1:
        {
            auto element (new (GC) mu::llvmc::skeleton::identity_element (most_specific_branch, source, &module_m->the_unit_type));
                ...
                if (predicate_offset == 2)
                {
                    ...
                    if (predicate != nullptr)
                    {
                        ...
                        if (integer_type != nullptr)
                        {
                            ...
                            {
                                result = true;
                                auto false_const (new (GC) mu::llvmc::skeleton::constant_integer (expression_a->region, &mu::llvmc::skeleton::branch::global, 1, 0));
                                auto true_const (new (GC) mu::llvmc::skeleton::constant_integer (expression_a->region, &mu::llvmc::skeleton::branch::global, 1, 1));
...
module_m (module_a),
function_m (new (GC) mu::llvmc::skeleton::function (node_a->region, &mu::llvmc::skeleton::branch::global)),
node_m (node_a),
