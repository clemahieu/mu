...
void mu::llvmc::module_processor::constant_pointer_null (mu::llvmc::ast::constant_pointer_null * constant_pointer_null)
{
	...
	if (type != nullptr)
	{
		...
		if (pointer != nullptr)
		{
			...
			auto skeleton (new (GC) mu::llvmc::skeleton::constant_pointer_null (constant_pointer_null->region, type));
        ...
        (mu::llvmc::ast::node * node_a, size_t index_a)
        {
			...
			if (result_a != nullptr)
			{
				...
				if (type != nullptr)
				{
					...
					if (value != nullptr)
					{
                        auto new_value (value->adapt_result (type, module_m.global_m.error, U"Actual result type does not match formal result type", mu::core::error_type::actual_formal_result_type_mismatch));
                        if (new_value != nullptr)
                        {
                            function_m->results.push_back (new (GC) mu::llvmc::skeleton::result (type, new_value));
	...
	if (pointer_type != nullptr)
	{
		...
		if (function_type != nullptr)
		{
			...
			for (; i != j && k != l && module_m.global_m.error == nullptr; ++i, ++k)
			{
				...
				{
					auto new_value (argument_value->adapt (function_type->function->parameters [i]->type (), module_m.global_m.error, U"Argument type does not match parameter type", mu::core::error_type::argument_type_does_not_match_parameter_type));
					*k = new_value;
				}
				else
				{
...
module_m (module_a),
function_m (new (GC) mu::llvmc::skeleton::function (node_a->region)),
node_m (node_a),
