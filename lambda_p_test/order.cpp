...
void lambda_p_test::order::run_1 ()
{
	...
	lambda_p::order order (routine->body);
...
void lambda_p_test::order::run_2 ()
{
	...
	c1->dependencies.push_back (boost::make_shared <lambda_p::parameters> ());
...
void lambda_p_test::order::run_3 ()
{
	...
	auto root (boost::shared_ptr <lambda_p::expression> (new lambda_p::expression (lambda_p::context ())));
	routine->body = root;
	auto parameters (boost::make_shared <lambda_p::expression> (lambda_p::context ()));
	parameters->dependencies.push_back (boost::make_shared <lambda_p::parameters> ());
	auto r1 (boost::shared_ptr <lambda_p::reference> (new lambda_p::reference (parameters, 0)));
