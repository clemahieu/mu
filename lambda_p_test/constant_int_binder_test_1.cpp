...
void lambda_p_test::constant_int_binder_test_1::run ()
{
	...
	instances [binder] = constant_int_binder;
	instances [number] = boost::shared_ptr <lambda_p_kernel::number> (new lambda_p_kernel::number (16));
	instances [bits] = boost::shared_ptr <lambda_p_kernel::number> (new lambda_p_kernel::number (64));
	...
	assert (boost::static_pointer_cast <lambda_p_llvm::value> (instances [declaration])->operator() () != NULL);
	assert (llvm::isa <llvm::ConstantInt> (boost::static_pointer_cast <lambda_p_llvm::value> (instances [declaration])->operator() ()));
	assert (llvm::cast <llvm::ConstantInt> (boost::static_pointer_cast <lambda_p_llvm::value> (instances [declaration])->operator() ())->getBitWidth () == 64);
	assert (llvm::cast <llvm::ConstantInt> (boost::static_pointer_cast <lambda_p_llvm::value> (instances [declaration])->operator() ())->getValue () == 16);
}