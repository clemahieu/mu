...
std::pair <size_t, std::vector <boost::shared_ptr <lambda_p::node>>> lambda_p_io::extensions::data::operator () (std::vector <boost::shared_ptr <lambda_p_io::ast::node>> & values_a, size_t position_a, boost::shared_ptr <lambda_p::errors::error_target> errors_a)
{
	auto data_position (position_a + 1);
	std::pair <size_t, std::vector <boost::shared_ptr <lambda_p::node>>> result;
	if (data_position < values_a.size ())
	{
		...
		{
			result.second.push_back (boost::shared_ptr <lambda_p::node> (new lambda_p::data (data_identifier->string)));
		}
		else
		{
