...
void lambda_p::core::implode::operator () (std::vector <boost::shared_ptr <lambda_p::core::node>> & parameters, boost::shared_ptr <lambda_p::core::target> target_a)
{
	assert (parameters.size () > 0);
	assert (parameters [0].get () == this);
	std::vector <boost::shared_ptr <lambda_p::core::node>> parameters_l (++parameters.begin (), parameters.end ());
