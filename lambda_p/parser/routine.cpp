...
size_t * lambda_p::parser::routine::sink_result (size_t declaration)
{
	routine_m->surface->results.push_back (declaration);
	return & (routine_m->surface->results [routine_m->surface->results.size () - 1]);
}

size_t * lambda_p::parser::routine::sink_argument (size_t argument)
{
	routine_m->surface->parameters.push_back (argument);
	return & (routine_m->surface->parameters [routine_m->surface->parameters.size () - 1]);
}