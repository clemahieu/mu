...
void lambda_p_test::cvalue_test::run_1 ()
{
	...
	list [2] = boost::shared_ptr <lambda_p::binder::node> (new lambda_p_kernel::nodes::data (std::wstring (L"a")));
	list [3] = boost::shared_ptr <lambda_p::binder::node> (new lambda_p_kernel::nodes::data (std::wstring (L"b")));
	...
	assert ((boost::dynamic_pointer_cast <lambda_p_kernel::nodes::data> (result [0]))->string ().compare (std::wstring (L"b")) == 0);
}
...
void lambda_p_test::cvalue_test::run_2 ()
{
	...
	assert (problems.errors.empty ());
	assert (result.nodes.size () == 1);
	assert ((boost::dynamic_pointer_cast <lambda_p_kernel::nodes::data> (result [0]))->string ().compare (std::wstring (L"a")) == 0);
}