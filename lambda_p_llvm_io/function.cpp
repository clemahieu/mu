...
lambda_p_llvm_io::function::function (lambda_p_llvm_io::expression & expression_a, boost::shared_ptr <lambda_p_llvm::function_pointer::node> function_a)
	: function_m (function_a),
	expression (expression_a)
{
	assert (function_m->value ()->getType ()->isPointerTy ());
	assert (llvm::cast <llvm::PointerType> (function_m->value ()->getType ())->getElementType ()->isFunctionTy ());
}

void lambda_p_llvm_io::function::operator () (boost::shared_ptr <lambda_p::errors::error_target> errors_a, llvm::BasicBlock * & context_a, std::vector <boost::shared_ptr <lambda_p::node>> & arguments_a)
{
	auto function_type (llvm::cast <llvm::FunctionType> (llvm::cast <llvm::PointerType> (function_m->value ()->getType ())->getElementType ()));
	if (arguments_a.size () == function_type->getNumParams ())
	{
		...
		if (good)
		{
			auto call (llvm::CallInst::Create (function_m->value (), arguments));
			context_a->getInstList ().push_back (call);
			if (function_m->multiple_return)
			{
