...
void lambda_p_script_test::package::get_recursive::run_2 ()
{
	...
	std::vector <boost::shared_ptr <lambda_p::node>> arguments;
	arguments.push_back (package);
	arguments.push_back (boost::shared_ptr <lambda_p::node> (new lambda_p_script::string::node (std::wstring (L"a"))));
	std::vector <boost::shared_ptr <lambda_p::node>> results;
	recursive.perform (errors, arguments, results);
	assert (errors->errors.empty ());
	assert (results.size () == 1);
...
void lambda_p_script_test::package::get_recursive::run_3 ()
{
	...
	arguments.push_back (package);
	arguments.push_back (boost::shared_ptr <lambda_p::node> (new lambda_p_script::string::node (std::wstring (L"a"))));
	arguments.push_back (boost::shared_ptr <lambda_p::node> (new lambda_p_script::string::node (std::wstring (L"b"))));
	std::vector <boost::shared_ptr <lambda_p::node>> results;
	recursive.perform (errors, arguments, results);
	assert (errors->errors.empty ());
	assert (results.size () == 1);
