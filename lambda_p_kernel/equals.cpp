...
void lambda_p_kernel::equals::bind (lambda_p::core::statement * statement, lambda_p::binder::list & nodes, lambda_p::errors::error_list & problems)
{
	...
	if (problems.errors.empty ())
	{
		...
		if (problems.errors.empty ())
		{
			boost::shared_ptr <lambda_p_kernel::bool_c> result (new lambda_p_kernel::bool_c (left->value == right->value));
			nodes [statement->association->declarations [0]] = result;
		}
	}
}

std::wstring lambda_p_kernel::equals::binder_name ()
{
	return std::wstring (L"equals");
}