...
void lambda_p_llvm_wrapper::module::get_package::operator () (boost::shared_ptr <lambda_p::errors::error_target> errors_a, lambda_p_script::segment <boost::shared_ptr <lambda_p::node>> parameters, std::vector <boost::shared_ptr <lambda_p::node>> & results)
{
	...
	if (one.get () != nullptr)
	{
		if (two.get () != nullptr)
		{
			...
			for (auto i (one->module->getFunctionList ().begin ()), j (one->module->getFunctionList ().end ()); i != j; ++i)
			{
				...
				package->items [wname] = boost::shared_ptr <lambda_p::node> (new lambda_p_llvm_wrapper::function::node (function, false));
...
size_t lambda_p_llvm_wrapper::module::get_package::count ()
{
	return 2;
}

std::wstring lambda_p_llvm_wrapper::module::get_package::name ()
{
	return std::wstring (L"lambda_p_llvm_wrapper::module::get_package");
}