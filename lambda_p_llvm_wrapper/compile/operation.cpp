...
void lambda_p_llvm_wrapper::compile::operation::operator () (boost::shared_ptr <lambda_p::errors::error_target> errors_a, lambda_p_script::segment <boost::shared_ptr <lambda_p::node>> parameters, std::vector <boost::shared_ptr <lambda_p::node>> & results)
{
	auto module (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::module::node> (parameters [0]));
		...
		if (name.get () != nullptr)
		{
			...
			if (error.empty ())
			{
				...
				if (link)
				{
					...
					command << "\"c:\\Program Files (x86)\\Microsoft Visual Studio 10.0\\VC\\vcvarsall.bat\" amd64 && link kernel32.lib /NOLOGO /SUBSYSTEM:WINDOWS /ENTRY:_start ";
					command << obj_name;
					command << " /OUT:";
					command << name->string;
					command << ".exe";
...
size_t lambda_p_llvm_wrapper::compile::operation::count ()
{
	return 2;
}

std::wstring lambda_p_llvm_wrapper::compile::operation::name ()
{
	return std::wstring (L"lambda_p_llvm_wrapper::compile::operation");
}