...
std::pair <size_t, std::vector <boost::shared_ptr <lambda_p::core::expression>>> lambda_p_serialization::extensions::data::operator () (std::vector <boost::shared_ptr <lambda_p_serialization::ast::node>> & values_a, size_t position_a, boost::shared_ptr <lambda_p::errors::error_target> errors_a)
{
	...
	if (data_position < values_a.size ())
	{
		...
		auto data_identifier (boost::dynamic_pointer_cast <lambda_p_serialization::ast::identifier> (data_node));
