...
void lambda_p_llvm_test::instruction_package::run_1 ()
{
	...
	source (L"[[:~; instruction insert block left right] [instruction left right; value] .id [insert block value] value]"); 
	source ();
	...
	a1.push_back (boost::make_shared <lambda_p_llvm::basic_block::node> (block));
	...
	assert (b2.errors->errors.empty ());
	assert (r2.size () == 1);
	assert (block->getInstList ().size () == 4);
}