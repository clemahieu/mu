...
void lambda_p_llvm::memcpy_binder::bind (::lambda_p::core::statement * statement, ::std::map < ::lambda_p::core::node *, ::boost::shared_ptr < ::lambda_p::binder::node_instance> > & instances, ::std::vector < ::boost::shared_ptr < ::lambda_p::errors::error> > & problems)
{
	check_count_only_references (0, 3, statement, problems);
	if (problems.empty ())
	{
		::boost::shared_ptr < ::lambda_p::binder::node_instance> destination_instance (instances [statement->association->parameters [0]]);
		...
		{
			::boost::shared_ptr < ::lambda_p::binder::node_instance> source_instance (instances [statement->association->parameters [1]]);
			::boost::shared_ptr < ::lambda_p_llvm::value> source (::boost::dynamic_pointer_cast < ::lambda_p_llvm::value> (source_instance));
			if (source.get () != NULL)
			{
				::boost::shared_ptr < ::lambda_p::binder::node_instance> length_instance (instances [statement->association->parameters [2]]);
				...
				if (length.get () != NULL)
				{
					if (destination->value_m->getType () == ::llvm::Type::getInt8PtrTy (context.context))
					{
						if (source->value_m->getType () == ::llvm::Type::getInt8PtrTy (context.context))
						{
							...
							{
								add_error (::std::wstring (L"memcpy_binder expects length to be an Int64"), problems);
							}
						}
						else
						{
							add_error (::std::wstring (L"memcpy_binder expects source to be an Int8Ptr"), problems);
						}
					}
					else
					{
						add_error (::std::wstring (L"memcpy_binder expects destination to be an Int8Ptr"), problems);
					}
				}
				else
				{
					add_error (::std::wstring (L"memcpy_binder expects length to be an llvm value"), problems);
				}
			}
			else
			{
				add_error (::std::wstring (L"memcpy_binder expects destination to be an llvm value"), problems);
			}
		}
		else
		{
...
::std::wstring lambda_p_llvm::memcpy_binder::binder_name ()
{
	return ::std::wstring (L"memcpy_binder");
}