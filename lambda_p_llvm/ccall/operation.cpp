...
void lambda_p_llvm::ccall::operation::perform (boost::shared_ptr <mu::core::errors::error_target> errors_a, mu::core::segment <boost::shared_ptr <mu::core::node>> parameters, std::vector <boost::shared_ptr <mu::core::node>> & results)
{
	if (parameters.size () > 2)
	{
		...
		auto three (boost::dynamic_pointer_cast <lambda_p_llvm::value::node> (parameters [2]));
		if (one.get () != nullptr)
		{
			if (two.get () != nullptr)
			{
				if (three.get () != nullptr)
				{
					...
					{
						auto bool_type (boost::dynamic_pointer_cast <lambda_p_llvm::integer_type::node> (one->type));
						if (bool_type.get () != nullptr)
						{
							if (bool_type->integer_type ()->getBitWidth () == 1)
							{
								...
								if (! (*errors_a) ())
								{
									...
									{
										auto i_value (boost::dynamic_pointer_cast <lambda_p_llvm::value::node> (*i));
										auto k_value (boost::dynamic_pointer_cast <lambda_p_llvm::value::node> (*k));
										if (i_value.get () != nullptr && k_value.get () != nullptr)
										{
											...
											if (i_type == k_type)
											{
												...
												results.push_back (boost::make_shared <lambda_p_llvm::value::node> (phi, i_value->type));
											}
											else
											{
