...
{
	bool problem (false);
	check_count (problem, 1, 2, L"type_binder", statement, problems);
	if (!problem)
	{
		::lambda_p::core::declaration * declaration (statement->results [0]);
		::lambda_p::core::node * context_node (statement->parameters [0]);
		...
		case ::lambda_p::core::node_reference:
			{
				...
				if (context.get () != NULL)
				{
					::lambda_p::core::node * command (statement->parameters [1]);
					...
					default:
						...
						problems << command->node_type_name ();
						problems << '\n';
					}
				}
				else
			...
			break;
		default:
			problems << L"type_binder expects argument 1 to be a reference to the context, have: ";
			problems << context_node->node_type_name ();
			problems << '\n';
		}
	}
}
