...
void lambda_p_llvm::instructions::lshr::operator () (boost::shared_ptr <lambda_p::errors::error_target> errors_a, lambda_p::segment <boost::shared_ptr <lambda_p::node>> parameters_a, std::vector <boost::shared_ptr <lambda_p::node>> & results_a)
{
	...
	if (one.get () != nullptr)
	{
		if (two.get () != nullptr)
		{
			...
			if (one_int && two_int)
			{
				...
				{
					auto instruction (llvm::BinaryOperator::CreateLShr (one->value (), two->value ()));
...
size_t lambda_p_llvm::instructions::lshr::count ()
{
	return 2;
}