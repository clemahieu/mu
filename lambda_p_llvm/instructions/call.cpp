...
void lambda_p_llvm::instructions::call::operator () (boost::shared_ptr <lambda_p::errors::error_target> errors_a, lambda_p::segment <boost::shared_ptr <lambda_p::node>> parameters, std::vector <boost::shared_ptr <lambda_p::node>> & results)
{
	auto one (boost::dynamic_pointer_cast <lambda_p_llvm::value::node> (parameters [0]));
	auto two (boost::dynamic_pointer_cast <lambda_p_script::values::operation> (parameters [1]));
	if (one.get () != nullptr)
	{
		if (two.get () != nullptr)
		{
			...
			if (pointer_type.get () != nullptr)
			{
				...
				if (function_type.get () != nullptr)
				{
					...
					size_t position (0);
					for (auto i (two->values.begin ()), j (two->values.end ()); i != j; ++i, ++position)
					{
...
size_t lambda_p_llvm::instructions::call::count ()
{
	return 2;
}