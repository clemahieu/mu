...
#include <lambda_p_llvm_wrapper/execution_engine/generic_value/node.h>
...
void lambda_p_llvm_io_test::synthesizer::run_1 ()
{
	...
	auto module (boost::make_shared <lambda_p_llvm_wrapper::module::node> (new llvm::Module (llvm::StringRef (""), context)));
	auto function (boost::make_shared <lambda_p_llvm_wrapper::function::node> (llvm::Function::Create (llvm::FunctionType::get (llvm::Type::getVoidTy (context), false), llvm::GlobalValue::ExternalLinkage), false));
	...
	auto f1 (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::function::node> (values [0]));
	assert (f1.get () != nullptr);
	lambda_p_llvm_wrapper::execution_engine::create_jit create_jit;
	...
	assert (v1.size () == 1);
	auto engine (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::execution_engine::node> (v1 [0]));
	assert (engine.get () != nullptr);
	lambda_p_llvm_wrapper::execution_engine::run_function run_function;
...
void lambda_p_llvm_io_test::synthesizer::run_2 ()
{
	...
	llvm::LLVMContext context;
	auto module (boost::make_shared <lambda_p_llvm_wrapper::module::node> (new llvm::Module (llvm::StringRef (""), context)));
	auto function (boost::make_shared <lambda_p_llvm_wrapper::function::node> (llvm::Function::Create (llvm::FunctionType::get (llvm::Type::getInt32Ty (context), false), llvm::GlobalValue::ExternalLinkage), false));
	auto block (llvm::BasicBlock::Create (context));
	function->function ()->getBasicBlockList ().push_back (block);
	...
	run_function.perform (builder.errors, a2, v2);
	assert (v2.size () == 1);
	auto res (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::execution_engine::generic_value::node> (v2 [0]));
...
void lambda_p_llvm_io_test::synthesizer::run_3 ()
{
	...
	auto function (boost::make_shared <lambda_p_llvm_wrapper::function::node> (llvm::Function::Create (llvm::FunctionType::get (llvm::Type::getInt32Ty (context), arg_types, false), llvm::GlobalValue::ExternalLinkage), false));
	auto block (llvm::BasicBlock::Create (context));
	function->function ()->getBasicBlockList ().push_back (block);
	...
	arguments.push_back (routine);
	arguments.push_back (module);
	arguments.push_back (boost::make_shared <lambda_p_llvm_wrapper::type::node> (llvm::Type::getInt32Ty (context)));
	std::vector <boost::shared_ptr <lambda_p::node>> values;
	synthesizer.perform (builder.errors, arguments, values);
	llvm::PassManager PM;
	std::string text;
	...
	a2.push_back (boost::make_shared <lambda_p_llvm_wrapper::execution_engine::generic_value::node> (val));
	run_function.perform (builder.errors, a2, v2);
	assert (v2.size () == 1);
	auto res (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::execution_engine::generic_value::node> (v2 [0]));
	assert (res.get () != nullptr);
...
void lambda_p_llvm_io_test::synthesizer::run_4 ()
{
	...
	auto function (boost::make_shared <lambda_p_llvm_wrapper::function::node> (llvm::Function::Create (llvm::FunctionType::get (res_type, false), llvm::GlobalValue::ExternalLinkage), true));
	auto block (llvm::BasicBlock::Create (context));
	function->function ()->getBasicBlockList ().push_back (block);
	...
	block->getInstList ().push_back (ret);
	module->module->getFunctionList ().push_back (function->function ());
	auto fun1 (boost::make_shared <lambda_p_llvm_wrapper::function::node> (llvm::Function::Create (llvm::FunctionType::get (llvm::Type::getInt32Ty (context), res_types, false), llvm::GlobalValue::ExternalLinkage), false));
	...
	run_function.perform (builder.errors, a2, v2);
	assert (v2.size () == 1);
	auto r (boost::dynamic_pointer_cast <lambda_p_llvm_wrapper::execution_engine::generic_value::node> (v2 [0]));
